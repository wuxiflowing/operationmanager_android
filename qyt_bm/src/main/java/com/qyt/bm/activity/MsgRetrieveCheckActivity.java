...
public class MsgRetrieveCheckActivity extends BaseActivity {
        ...
        if (!TextUtils.isEmpty(d.txtMessageState)) {
            switch (d.txtMessageState) {
                ...
                case "auditReject":
                    ...
                    break;
                case "magAuditPass":
                    installTaskState.setText("大区审核通过");
                    orderItems.add("发起人：" + beginp);
                    orderItems.add("审核人：" + d.txtCenMagName);
                    orderItems.add("回收原因：" + d.tarDmtRemaks);
                    orderItems.add("备注：" + reason);
                    break;
                case "magAuditReject":
                    installTaskState.setText("大区审核驳回");
                    orderItems.add("发起人：" + beginp);
                    orderItems.add("审核人：" + d.txtCenMagName);
                    orderItems.add("回收原因：" + d.tarDmtRemaks);
                    orderItems.add("备注：" + reason);

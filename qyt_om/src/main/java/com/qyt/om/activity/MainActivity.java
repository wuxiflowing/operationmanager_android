...
import com.qyt.om.R;
import com.qyt.om.activity.fragment.DeviceFragment;
import com.qyt.om.activity.fragment.MessageFragment;
import com.qyt.om.activity.fragment.MineFragment;
import com.qyt.om.activity.fragment.TaskFragment;
import com.qyt.om.base.BaseActivity;
import com.qyt.om.comm.HttpConfig;
import com.qyt.om.response.VersionInfo;
import com.qyt.om.service.GtuiIntentService;
import com.qyt.om.service.GtuiPushService;
...
public class MainActivity extends BaseActivity implements DownLoadImpl {
    ...
    private int[] images = new int[]{
            R.drawable.tabbar_message, R.drawable.tabbar_task, R.drawable.tabbar_device, R.drawable.tabbar_mine
    };
...
//        setMessageCount(22);
    ...
    private void downLoadApk(String url) {
        this.bindService(url, "yunweiguanjia.apk");
        progressDialog = new DownloadDialog(this, "下载中", "后台下载", "取消", new DownloadDialog.DialogConfirmListener() {
            @Override
            public void onDialogConfirm(boolean result, Object value) {
                if (result) {
                    Uri fileUri = FileProvider.getUriForFile(mContext, "com.qyt.om.fileprovider", (File) value);
            ...
            downloadService.setOnProgressListener(new DownloadService.OnProgressListener() {
                ...
                public void onProgressDone(File downloadFile) {
                    ...
                    if (progressDialog.isShowing()) {
                        ...
                        Uri fileUri = FileProvider.getUriForFile(mContext, "com.qyt.om.fileprovider", downloadFile);
        ...
        switch (n) {
            ...
            case 1:
                if (taskFragment == null) {
                    ...
                    fts.add(R.id.main_content, taskFragment, tabTitles[n]);
                }
                fts.show(taskFragment).commit();
                break;
            case 2:
                if (deviceFragment == null) {
                    deviceFragment = new DeviceFragment();
                    fts.add(R.id.main_content, deviceFragment, tabTitles[n]);
                }
                fts.show(deviceFragment).commit();
                break;
            case 3:
    ...
    private void hideFragment(FragmentTransaction fts) {
        ...
        if (taskFragment != null)
            fts.hide(taskFragment);
        if (deviceFragment != null)
            fts.hide(deviceFragment);
        if (mineFragment != null)
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            if ((System.currentTimeMillis() - mExitTime) > 2000) {
                ...
                OmApplication.getInstance().exitApplication();
            }
            return true;
        }
